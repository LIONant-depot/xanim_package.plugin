...
int main( int argc, const char* argv[] )
{
   ...
   //
    auto AnimPackageCompilerPipeline = xanim_package_compiler::instance::Create();
    ...
    if constexpr (false)
    {
        ...
        , "D1"
        , "-DESCRIPTOR"
        , "Descriptors\\AnimPackage\\03\\40\\949478640AFE4003.desc"
        , "-OUTPUT"
    ...
    // Parse parameters
    //
    if (auto Err = AnimPackageCompilerPipeline->Parse(argc, argv); Err)
    {
    ...
    // Start compilation
    //
    if (auto Err = AnimPackageCompilerPipeline->Compile(); Err)
    {
        Err.ForEachInChain([&](xerr Error)
        {
            auto Hint = Err.getHint();
